...
app.controller('Student', ["$scope", "$rootScope","$timeout", "$filter", "Selects", function ($s, $rootScope,$timeout,$filter, Selects) {
    ...
    $s.notByIdentity=function(){
        ...
        if(isFinaConfirm({
            ...
            Layer.alert({width:350,height:170,type:"msg",title:"你要处理的认证申请里有已认证<br>的申请，不能逆操作"});
            return false;
        };
        /*确认不通过*/
    ...
    $s.successIdentity=function(){
        ...
        if(isFinaConfirm({
            ...
            Layer.alert({width:350,height:170,type:"msg",title:"你要处理的认证申请里有已认证<br>的申请，不能重复操作"});
            return false;
        };
