...
$s.getDataStatistics=function(){
	$.AJAX({
		...
		success:function(data){
			...
			//统计table数据列表
			$s.tabDataList=[
				getDataFromJson(['流水统计','现约进账(元)','报名进账(元)','合计进账(元)'],['dateStr','instantOrderMoney','signupOrderMoney','totalMoney'],$s.dataList),
				getDataFromJson(['订单统计','预约单量(单)','现约单量(单)','现约成功付费单量'],['dateStr','planOrderNum','instantOrderNum','instantPayOrderNum'],$s.dataList),
